    ...
    impl MSPObjWrapped<Galactic> for Galactic {
        ...
        fn perform(&self, _ins: &[&[f64]], outs: &mut [&mut [f64]], _nframes: usize) {
            ...
            while counter < _nframes {
                ...
                let dither = |input_sample: f64, fpd: &mut u32| -> f64 {
                    let (_mantissa, expon, _sign) = Float::integer_decode(input_sample);
                    *fpd ^= *fpd << 13;
                    *fpd ^= *fpd >> 17;
                    *fpd ^= *fpd << 5;
                    (*fpd).wrapping_sub(i32::MAX as u32) as f64 * 1.1e-44 * (expon + 62).pow(2) as f64
                };

                outs[0][counter] = dither(input_sample_l, &mut g.fpd_l);
                outs[1][counter] = dither(input_sample_r, &mut g.fpd_r);
